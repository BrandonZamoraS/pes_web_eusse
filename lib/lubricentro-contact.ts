...
export type LubricentroContactPayload = {
  name: string;
  service: string;
  phone: string;
  email: string;
...
export type LubricentroContactErrors = Partial<
  Record<keyof LubricentroContactPayload, string>
>;
...
type BuildMailgunMessageInput = {
  fromEmail: string;
  toEmail: string;
  payload: LubricentroContactPayload;
};
...
export function normalizeLubricentroContactPayload(
  payload: Partial<LubricentroContactPayload>,
): LubricentroContactPayload {
  return {
...
export function validateLubricentroContactPayload(
  payload: LubricentroContactPayload,
): LubricentroContactErrors {
  const errors: LubricentroContactErrors = {};
...
export function buildMailgunMessage({
  ...
  return {
    from: fromEmail,
    to: toEmail,
    subject: "Nuevo lead de Lubricentro Ginni",
    text: [
      "Nuevo lead desde el formulario de Lubricentro Ginni.",
      "",
      `Nombre: ${payload.name}`,
...
export function getMailgunConfig(
  ...
  const toEmail = env.LUBRICENTRO_CONTACT_TO_EMAIL?.trim();
  const apiBaseUrl =
